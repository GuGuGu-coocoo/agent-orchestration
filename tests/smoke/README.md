...
| --- | --- | --- |
| `run-offline.sh` | script-level contracts with fake workers: doctor, frontmatter, fail-fast, dry-run, status, collect-result, archive, worker-notify (message wording per exit code, fake-codex delivery, NOTIFY_FAILED hints, exit 14/15, detached notify), session identity (explicit flag / `CODEX_THREAD_ID` / archived / refusal to guess), the run-worker safety harness (fake opencode: report validation, stale-report quarantine, lock identity incl. surviving worker pid and `--break-lock`, **real SIGTERM cancellation keeps the lock**, TASK.md + list-placeholder + REVIEW.md validation, dirty baseline + BASELINE.patch, cwd pinning), check-state fail-closed matrix (empty/invalid/null/missing queue, escalated/checkpoint verdicts, unacceptable reports, live worker_pid), the phase driver obeying a checkpoint verdict, install/uninstall boundaries (fresh HOME, symlink escape with **no target created**, source-copy refusal), sibling skills untouched | no |
